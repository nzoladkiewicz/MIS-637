'''
k-Nearest Neighbors: Fit - Having explored the Congressional voting records dataset, it is time now to build your first classifier. 
...
'''
k-Nearest Neighbors: Predict - Having fit a k-NN classifier, you can now use it to predict the label of a new data point. 
...
'''
The digits recognition dataset - Up until now, you have been performing binary classification, since the target variable had two possible outcomes. 
Hugo, however, got to perform multi-class classification in the videos, where the target variable could take on three possible outcomes. 
...
Because it is a 2D array of the images corresponding to each sample, this 'images' key is useful for visualizing the images, as you'll see in this exercise (for more on plotting 2D arrays, see Chapter 2 of DataCamp's course on Data Visualization with Python). 
...
print("Description of the digits dataset: ", digits.DESCR)
...
'''
Train/Test Split + Fit/Predict/Accuracy - Now that you have learned about the importance of splitting your data into training and test sets, it's time to practice doing this on the digits dataset! 
...
'''
Overfitting and underfitting - Remember the model complexity curve that Hugo showed in the video? 
